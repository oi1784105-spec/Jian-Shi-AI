...
from app.services.client.ai_provider_runtime import AIProviderRuntime
...
class AIService:
    ...
    def __init__(self, runtime: AIProviderRuntime):
        self.runtime = runtime

    async def _chat(self, messages: list, temperature: float = 0.7, max_tokens: int = 1200) -> str:
        """基础对话补全调用"""
        try:
            return await self.runtime.chat(messages, temperature=temperature, max_tokens=max_tokens)
        except Exception as e:
            logger.error("AI API 调用失败 provider=%s error_type=%s", self.runtime.config.provider, type(e).__name__)
            raise

    async def _chat_stream(self, messages: list, temperature: float = 0.7, max_tokens: int = 800):
        """流式对话补全调用，逐块返回文本"""
        try:
            async for chunk in self.runtime.chat_stream(messages, temperature=temperature, max_tokens=max_tokens):
                yield chunk
        except Exception as e:
            logger.error("AI 流式 API 调用失败 provider=%s error_type=%s", self.runtime.config.provider, type(e).__name__)
            raise
    ...
    async def parse_and_analyze_resume(self, resume_text: str, target_position: str) -> tuple[dict, dict]:
        """Parse and evaluate a resume in one provider request."""
        is_intern = any(keyword in target_position.lower() for keyword in ["实习", "intern"])
        evaluation_standard = (
            "目标是实习岗位，请按在校学生标准评价，不因缺少正式工作或生产经验扣分，"
            "重点看技术基础、项目完成度和学习能力。"
            if is_intern
            else "目标是正式岗位，请重点评价工作经验、项目深度和解决复杂问题的能力。"
        )
        messages = [
            {
                "role": "system",
                "content": (
                    "你是资深技术招聘专家。请一次完成简历结构化提取和岗位匹配分析。"
                    f"{evaluation_standard}"
                    "必须只返回一个合法 JSON 对象，不要使用 Markdown。格式如下："
                    '{"parsed_content":{"name":"姓名","education":"学历信息",'
                    '"skills":["技能"],"experience":["工作或实习经历"],'
                    '"projects":["项目经历"],"summary":"候选人背景概述"},'
                    '"analysis":{"overall_score":7.5,"strengths":["优势"],'
                    '"weaknesses":["不足"],"suggestions":["改进建议"],'
                    '"keyword_match":["匹配技能"],"missing_keywords":["缺少技能"],'
                    '"summary":"100字以内的质量总结"}}。'
                    "评分范围为 1 到 10；每个列表保留最重要的 2 到 3 项，内容简洁具体。"
                ),
            },
            {
                "role": "user",
                "content": f"目标岗位：{target_position}\n简历内容：\n{resume_text}",
            },
        ]
        result = await self._chat(messages, temperature=0.3, max_tokens=1800)
        payload = self._extract_json(result)
        if not isinstance(payload, dict):
            raise ValueError("Resume analysis response must be a JSON object")
        parsed = payload.get("parsed_content")
        analysis = payload.get("analysis")
        if not isinstance(parsed, dict) or not isinstance(analysis, dict):
            raise ValueError("Resume analysis response is missing required fields")
    ...
    async def parse_resume(self, resume_text: str) -> dict:
        ...
        result = await self._chat(messages, temperature=0.3, max_tokens=1200)
    ...
    async def analyze_resume(self, parsed_resume: dict, target_position: str) -> dict:
        ...
        result = await self._chat(messages, temperature=0.4, max_tokens=1200)
        return self._extract_json(result)
    ...
    async def generate_questions(
        ...
        result = await self._chat(messages, temperature=0.7, max_tokens=1400)
        return self._extract_json(result)
    ...
    async def evaluate_answer(
        ...
        result = await self._chat(messages, temperature=0.5, max_tokens=800)
        return self._extract_json(result)
    ...
    async def evaluate_answer_stream(
        ...
        async for chunk in self._chat_stream(messages, temperature=0.5, max_tokens=800):
            yield chunk
    ...
    async def generate_report(
        ...
        result = await self._chat(messages, temperature=0.5, max_tokens=1000)
        return self._extract_json(result)
