...
class InterviewService:
    ...
    async def submit_answer(
        ...
        provider = await ai_provider_service._owned(db, user_id, interview.ai_provider_id) if interview.ai_provider_id else None
        if not provider or not provider.is_validated:
            raise ValidationError(message="该面试使用的 API 配置已不可用")
        ai = AIService(ai_provider_service.runtime(provider, interview.ai_model))
    ...
    async def submit_answer_stream(
        ...
        provider = await ai_provider_service._owned(db, user_id, interview.ai_provider_id) if interview.ai_provider_id else None
        if not provider or not provider.is_validated:
            yield f"data: {json.dumps({'type': 'error', 'content': '该面试使用的 API 配置已不可用'}, ensure_ascii=False)}\n\n"
            return
        ai = AIService(ai_provider_service.runtime(provider, interview.ai_model))
        ...
        # 流式评估回答
        full_text = ""
        async for chunk in ai.evaluate_answer_stream(
            question=current_question,
            answer=answer,
            resume_context=parsed_resume,
            chat_history=chat_history
        ...
        if is_finished:
            ...
            if background_tasks is not None:
                background_tasks.add_task(
                    InterviewService.generate_report_background,
                    user_id,
                    interview_id,
                )
    ...
    @staticmethod
    async def generate_report_background(user_id: int, interview_id: int) -> None:
        """Generate the final report after the final-answer response has closed."""
        async with async_session() as db:
            try:
                ...
                parsed_resume = json.loads(resume.parsed_content) if resume and resume.parsed_content else {}
                provider = await ai_provider_service._owned(db, user_id, interview.ai_provider_id)
                ai = AIService(ai_provider_service.runtime(provider, interview.ai_model))
                ...
                answers = {
                    message.question_index: message
                    for message in result.scalars().all()
                    if message.role == "candidate" and message.question_index is not None
                }
                questions = interview.questions_data or []
                qa_data = [
                    {
                        "question": question["question"],
                        "answer": answers[index].content if index in answers else "未回答",
                        "score": float(answers[index].score) if index in answers and answers[index].score is not None else 0,
                    }
                ...
                report["question_scores"] = [
                    {"question": question["question"], "score": qa["score"], "feedback": ""}
                    for question, qa in zip(questions, qa_data)
                ]
                interview.report = json.dumps(report, ensure_ascii=False)
                await db.commit()
            except Exception:
                logger.exception("Background report generation failed interview_id=%s", interview_id)
                await db.rollback()
                interview = await db.scalar(
                    select(Interview).where(
                        Interview.id == interview_id,
                ...
                if interview and not interview.report:
                    interview.report = json.dumps(
                        {"summary": "报告生成失败", "strengths": [], "weaknesses": [], "suggestions": []},
                        ensure_ascii=False,
                    )
                    await db.commit()
    ...
    async def get_report(
        ...
        if interview.report:
            try:
                ...
                report = {"summary": "报告数据异常", "strengths": [], "weaknesses": [], "suggestions": []}
