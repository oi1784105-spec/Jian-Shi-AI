...
async def create_first_admin():
    ...
    # 管理员信息从本地环境变量读取，避免在仓库中公开凭据
    email = settings.ADMIN_EMAIL
    password = settings.ADMIN_PASSWORD
    first_name = "JianShi AI"
    last_name = "Admin"
    ...
    async with AsyncSession(engine) as session:
        ...
        if existing_admin:
            # Keep the database credentials in sync with the local .env file.
            existing_admin.password = Admin.get_password_hash(password)
            existing_admin.role = "superadmin"
            existing_admin.is_active = True
            await session.commit()
            print("✅ 超级管理员账号已更新！")
            print(f"   邮箱: {email}")
            print("   密码: 已与 .env 中的 ADMIN_PASSWORD 同步")
            return
