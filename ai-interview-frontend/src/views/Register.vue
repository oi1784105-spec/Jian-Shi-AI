<template>
  <div class="auth-page">
    ...
    <div class="auth-layout">
      <section class="auth-intro"><div class="auth-brand"><span class="brand-mark"><Sparkles :size="17" /></span><span>{{ t('common.brand') }}</span></div><p class="eyebrow">{{ t('common.product') }}</p><h1>{{ t('auth.createAccount') }}</h1><p>{{ t('auth.subtitle') }}</p><div class="intro-rule"></div><span>02 / 03</span></section>
      <section class="auth-card card">
        <div class="auth-card-header"><p class="eyebrow">CREATE ACCOUNT</p><h2>{{ t('auth.signUp') }}</h2></div>
        <form @submit.prevent="handleRegister">
          <div class="form-row"><div class="form-group"><label>{{ t('auth.firstName') }}</label><input v-model="form.first_name" :placeholder="t('auth.firstName')" required /></div><div class="form-group"><label>{{ t('auth.lastName') }}</label><input v-model="form.last_name" :placeholder="t('auth.lastName')" required /></div></div>
          <div class="form-group"><label>{{ t('auth.email') }}</label><input v-model="form.email" type="email" :placeholder="t('auth.emailPlaceholder')" required /></div>
          <div class="form-group"><label>{{ t('auth.password') }}</label><input v-model="form.password" type="password" :placeholder="t('auth.passwordHint')" required /></div>
          <p v-if="error" class="error form-message">{{ error }}</p><p v-if="success" class="success form-message">{{ success }}</p>
          <button type="submit" class="btn-primary auth-submit" :disabled="loading">{{ loading ? t('auth.signingUp') : t('auth.signUp') }}<ArrowUpRight :size="17" /></button>
        </form>
        <p class="auth-link">{{ t('auth.hasAccount') }} <router-link to="/login">{{ t('auth.signInNow') }}</router-link></p>
      </section>
    </div>
  </div>
</template>
...
const form = reactive({ first_name: '', last_name: '', email: '', password: '' }); const error = ref(''); const success = ref(''); const loading = ref(false)
async function handleRegister() { error.value = ''; success.value = ''; loading.value = true; try { const res = await register(form); authStore.setAuth(res); success.value = t('auth.registerSuccess'); setTimeout(() => router.push('/dashboard'), 800) } catch (e) { error.value = e.message } finally { loading.value = false } }
</script>
